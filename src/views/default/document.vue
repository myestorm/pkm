        ...
        <pkm-space direction="vertical" fill class="header">
          ...
          <div class="pkm-totonoo-flex">
            ...
            <pkm-breadcrumb flex="auto" :max-count="3" :style="{ fontSize: `14px` }">
              <pkm-breadcrumb-item>
                <router-link to="/document"><icon-folder /></router-link>
              </pkm-breadcrumb-item>
              <pkm-breadcrumb-item v-for="item in breadcrumbs" :key="item.url">
                <router-link :to="`/document/${item.url}`">{{ item.title }}</router-link>
              </pkm-breadcrumb-item>
              <template #separator>
                <icon-right />
              </template>
          ...
          <pkm-input-search v-model="keyword" class="search-input" placeholder="试试搜索文档" :allow-clear="true" :loading=" searchLoading" @input="searchHandler" @search="searchHandler" @clear="searchClear" />
        </pkm-space>
      ...
      <pkm-layout-content class="pkm-totonoo-file-content">
        <div class="pkm-totonoo-markdown-editor" v-if="pageInfo.id">
          <div class="pkm-totonoo-flex header">
            ...
            <div class="action">
              <pkm-button-group>
                <pkm-button type="secondary" @click="sortEditHandler(pageInfo.id)">
                  <template #icon>
                    <icon-storage />
                  </template>
                  属性
                </pkm-button>
                <pkm-button type="primary" @click="updateContent">
                  <template #icon>
                    <icon-save />
                  </template>
                  保存
          ...
          <markdown-editor v-model="editorValue" @toolbarItemAction="saveEditor" height="calc(100vh - 98px)" />
        </div>
      </pkm-layout-content>
    </pkm-layout>
  </pkm-spin>
...
export default defineComponent({
  ...
  setup () {
    ...
    const linkDocument = (_paths: string[]) => {
      router.push(`/document/${_paths.join('/')}`)
    }
    const backToHandler = () => {
    ...
    const sortDragEndHandler = (data: {
      ...
      if (dropPosition === 0) { // 转移目录
        ...
        documentStore.documentMove({ id: dragItem.id, directory: _directory }).then(_ => {
          getList()
        }).catch(err => {
        ...
        documentStore.documentOrder(dragItem.id, _order).then(_ => {
          getList()
        }).catch(err => {
          msg.error(err.message)
        })
    ...
    const sortItemClick = (item: TypesDocument.IDocumentType) => {
    ...
    const clipboard = ref<TypesDocument.IDocumentType | null>(null)
      ...
      if (clipboard.value?.id) {
        switch (clipboardType.value) {
          ...
          case TypesBase.IClipboardType.COPY: {
            ...
            documentStore.documentCopy(postData).then(_ => {
              clipboard.value = null
              clipboardType.value = TypesBase.IClipboardType.NONE
              getList()
            }).catch(err => {
    ...
    const updateContent = () => {
      loading.value = true
      documentStore.documentUpdateContent(pageInfo.value.id, editorValue.value).then(res => {
        msg.success('保存成功')
      }).catch(err => {
        msg.error(err.message)
      }).finally(() => {
